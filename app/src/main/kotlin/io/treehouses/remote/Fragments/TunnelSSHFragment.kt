...
class TunnelSSHFragment : BaseFragment(), View.OnClickListener {
    ...
    private fun initializeDialog3(builder:AlertDialog.Builder, position:Int){
        ...
        builder.setPositiveButton("Confirm") { dialog, _ ->
            ...
            fun getListenerCommand():String{return getString(R.string.TREEHOUSES_SSHTUNNEL_REMOVE_PORT, portsName!![position].split(":".toRegex(), 2).toTypedArray()[0] + " " + hostsName!![myPos].split(":")[0])}
            listener.sendMessage(getListenerCommand())
...
//            listener.sendMessage("treehouses sshtunnel notice")
    ...
    private fun getMessage3(readMessage: String) {
        if(readMessage.contains("the command 'treehouses sshtunnel ports' returns nothing")){
