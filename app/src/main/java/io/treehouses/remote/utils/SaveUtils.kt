...
object SaveUtils {
    ...
    private fun getStringArray(context: Context, arrayName: String): ArrayList<String> {
        ...
        strArr = if (str == null || str == "") {
            ...
            str.split(DELIMITER).toTypedArray()
        }
    ...
    fun getCommandsList(context: Context): List<CommandListItem> {
        ...
        val finalArray: MutableList<CommandListItem> = ArrayList<CommandListItem>()
        if (titles.isEmpty() || values.isEmpty()) {
            return ArrayList<CommandListItem>()
        }
    ...
    fun getProfiles(context: Context): HashMap<String, List<NetworkProfile>> {
        ...
        val wifi: ArrayList<NetworkProfile> = ArrayList<NetworkProfile>()
        val hotspot: ArrayList<NetworkProfile> = ArrayList<NetworkProfile>()
        val bridge: ArrayList<NetworkProfile> = ArrayList<NetworkProfile>()
        for (i in json_profiles.indices) {
            val profile: NetworkProfile = gson.fromJson(json_profiles[i], NetworkProfile::class.java)
            if (profile.isWifi()) {
                wifi.add(profile)
        ...
        val profiles: HashMap<String, List<NetworkProfile>> = HashMap<String, List<NetworkProfile>>()
    ...
    fun deleteProfile(context: Context, groupPosition: Int, childPosition: Int) {
        val profiles: HashMap<String, List<NetworkProfile>> = getProfiles(context)
        val networkProfile: NetworkProfile = profiles[HomeFragment.group_labels[groupPosition]]!![childPosition]
        val gson = Gson()
