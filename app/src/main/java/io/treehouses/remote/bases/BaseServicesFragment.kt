...
class BaseServicesFragment : BaseFragment() {
    ...
    protected fun openLocalURL(url: String) {
        ...
        if (intent.resolveActivity(context!!.packageManager) != null) startActivity(chooser)
    }

    protected fun openTorURL(url: String) {
        val intent = context!!.packageManager.getLaunchIntentForPackage("org.torproject.torbrowser")
        if (intent != null) {
    ...
    private fun constructServiceList(servicesData: ServicesData?, services: ArrayList<ServiceInfo>) {
        ...
        services.clear()
        for (service in servicesData.getAvailable()) {
            if (inServiceList(service, services) == -1) {
                services.add(ServiceInfo(service, ServiceInfo.SERVICE_AVAILABLE, servicesData.getIcon().get(service),
                        servicesData.getInfo().get(service), servicesData.getAutorun().get(service)))
            }
        }
        for (service in servicesData.getInstalled()) {
        ...
        for (service in servicesData.getRunning()) {
            if (inServiceList(service, services) == -1) continue
    ...
    protected fun inServiceList(name: String?, services: ArrayList<ServiceInfo>): Int {
        for (i in services.indices) {
            if (services[i].name.equals(name)) return i
        }
        return -1
    }
