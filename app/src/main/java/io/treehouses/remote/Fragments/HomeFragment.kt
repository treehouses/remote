...
class HomeFragment : BaseHomeFragment(), SetDisconnect {
    ...
    private fun switchProfile(profile: NetworkProfile?) {
        ...
        when {
            profile.isWifi -> {
                //WIFI
                listener.sendMessage(getString(R.string.TREEHOUSES_WIFI, profile.ssid, profile.password))
                network_ssid = profile.ssid
            }
            profile.isHotspot -> {
                //Hotspot
                listener.sendMessage(getString(R.string.TREEHOUSES_AP, profile.option, profile.ssid, profile.password))
                network_ssid = profile.ssid
            }
            profile.isBridge -> {
                //Bridge
