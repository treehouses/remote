...
import io.treehouses.remote.SSH.StrictModeSetup;
...
public class SSHConsole extends AppCompatActivity implements BridgeDisconnectedListener {
    ...
    public void onCreate(Bundle icicle) {
        ...
        this.setContentView(R.layout.activity_ssh_console);
        ...
        booleanYes.setOnClickListener(v -> {
            PromptHelper helper = getCurrentPromptHelper();
        ...
        booleanNo.setOnClickListener(v -> {
            PromptHelper helper = getCurrentPromptHelper();
            if (helper == null) return;
        ...
        mKeyboardButton.setOnClickListener(view -> {
...
//        tabs = findViewById(R.id.tabs);
        ...
        pager.setOnClickListener(v -> showEmulatedKeys(true));
...
//                mKeyboardButton.setContentDescription(getResources().getText(R.string.image_description_show_keyboard));
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (!sessionOpen && disconnected)
            disconnect.setTitle("Close Console");
        disconnect.setEnabled(activeTerminal);
        disconnect.setIcon(android.R.drawable.ic_menu_close_clear_cancel);
        disconnect.setOnMenuItemClickListener(item -> {
        ...
        paste.setOnMenuItemClickListener(item -> {
            pasteIntoTerminal();
            return true;
        });
        ...
        urlscan.setOnMenuItemClickListener(item -> {
            final TerminalView terminalView = adapter.getCurrentTerminalView();

            List<String> urls = terminalView.bridge.scanForURLs();

            Dialog urlDialog = new Dialog(SSHConsole.this);
            urlDialog.setTitle("Scan for URLs");

            ListView urlListView = new ListView(SSHConsole.this);
            URLItemListener urlListener = new URLItemListener(SSHConsole.this);
            urlListView.setOnItemClickListener(urlListener);

            urlListView.setAdapter(new ArrayAdapter<>(SSHConsole.this, android.R.layout.simple_list_item_1, urls));
        ...
        resize = menu.add("Resize");
        if (hardKeyboard)
            resize.setAlphabeticShortcut('s');
        resize.setIcon(android.R.drawable.ic_menu_crop);
        resize.setEnabled(sessionOpen);
//        resize.setOnMenuItemClickListener(item -> {
//            final TerminalView terminalView = adapter.getCurrentTerminalView();
//
//            @SuppressLint("InflateParams")  // Dialogs do not have a parent view.
//            final View resizeView = inflater.inflate(R.layout.dia_resize, null, false);
//            new androidx.appcompat.app.AlertDialog.Builder(ConsoleActivity.this, R.style.AlertDialogTheme)
//                    .setView(resizeView)
//                    .setPositiveButton(R.string.button_resize, (dialog, which) -> {
//                        int width, height;
//                        try {
...
//		portForward.setEnabled(sessionOpen && canForwardPorts);
    ...
    private void onTerminalChanged() {
        ...
        updateDefault();
        updatePromptVisible();
        ActivityCompat.invalidateOptionsMenu(SSHConsole.this);
    }
    ...
    public class TerminalPagerAdapter extends PagerAdapter {
        ...
        public CharSequence getPageTitle(int position) {
            ...
            return "Treehouses Remote: " + bridge.host.getNickname();
        }
