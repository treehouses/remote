...
//		}
		...
		Thread connectionThread = new Thread(() -> transport.connect());
	...
	public void injectString(final String string) {
		...
		Thread injectStringThread = new Thread(() -> {
			try {
...
//		else
	...
	public void dispatchDisconnect(boolean immediate) {
		...
		if (immediate || (host.getQuickDisconnect() && !host.getStayConnected())) {
			...
			{
				final String line = "Connection Lost";
	...
	private void triggerDisconnectListener() {
		if (disconnectListener != null) {
			...
			new Handler(Looper.getMainLooper()).post(() -> disconnectListener.onDisconnected(TerminalBridge.this));
		}
	}
